...
pub struct MatchEEPosiXYDoF {
    pub arm_idx: usize,
}
impl MatchEEPosiXYDoF {
    pub fn new(arm_idx: usize) -> Self {Self{arm_idx}}
}
impl ObjectiveTrait for MatchEEPosiXYDoF {
    fn call(&self, x: &[f64], v: &vars::RelaxedIKVars, frames: &Vec<(Vec<nalgebra::Vector3<f64>>, Vec<nalgebra::UnitQuaternion<f64>>)>) -> f64 {
        ...
        let dist: f64 = (T_gc[0].powi(2) + T_gc[1].powi(2)).sqrt();

        let bound =  v.tolerances[self.arm_idx][0];
...
impl ObjectiveTrait for MatchEERotaDoF {
    fn call(&self, x: &[f64], v: &vars::RelaxedIKVars, frames: &Vec<(Vec<nalgebra::Vector3<f64>>, Vec<nalgebra::UnitQuaternion<f64>>)>) -> f64 {
        ...
        let angle: f64 = scaled_axis[self.axis].abs();
